 ...
 * @author Charles Xie
 */
public class HelperNoGUI {

    private static ScriptDialogNoGUI scriptDialog;

    private HelperNoGUI() {
    }

    static void showScriptDialog(System2DNoGUI box) {
        if (scriptDialog != null && scriptDialog.isShowing()) {
            scriptDialog.toFront();
        } else {
            scriptDialog = new ScriptDialogNoGUI(box);
            scriptDialog.pack();
